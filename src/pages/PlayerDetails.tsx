      ...
      <div className="relative md:h-[auto] rounded-2xl overflow-hidden group border border-white/10 pb-8">
        ...
        <div className="relative pt-[80px] px-8 flex flex-col md:flex-row items-end justify-between w-full h-full gap-6">
            <div className="flex flex-col md:flex-row items-center md:items-end gap-6 w-full text-center md:text-left">
                <div className="w-24 h-24 md:w-32 md:h-32 rounded-2xl bg-gradient-to-br from-[#cfbe94] to-[#d8d0b6] flex shrink-0 items-center justify-center text-5xl md:text-6xl font-black text-white shadow-xl shadow-[#cfbe94]/20 border-4 border-[#050507]">
                ...
                <div className="w-full">
                    ...
                    <div className="flex flex-wrap justify-center md:justify-start items-center gap-x-6 gap-y-2 mt-3 text-white/50 text-sm font-medium">
                        ...
                        {(!isEditing && displayedProfile.gdUsername) && (
                          <span className="flex items-center gap-1.5 text-[#d2c89e] bg-[#d2c89e]/10 px-2 py-0.5 rounded">
                    ...
                    {isEditing && (
                      <div className="mt-6 bg-black/40 backdrop-blur-md p-6 rounded-xl border border-white/10 text-left space-y-4 max-w-2xl">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div>
                            <label className="block text-xs uppercase font-bold text-white/50 mb-1 text-[#d2c89e]">GD Username</label>
                        ...
                        <div className="flex gap-2 justify-end">
                           ...
                           <button onClick={handleSaveProfile} disabled={saving} className="px-4 py-2 text-sm font-bold text-white bg-[#bfae7d] hover:bg-[#d8d0b6] rounded-lg transition-colors flex items-center gap-2">
                 ...
                 {activeTab === "slayer" ? (
                   <>
                     <div className="flex flex-col">
                        ...
                        <span className="text-2xl font-black text-[#cfbe94] mt-1">{player.points.toLocaleString()}</span>
                     </div>
                     <div className="flex flex-col">
                   ...
                   <div className="flex flex-col">
                      ...
                      <span className="text-2xl font-black text-[#cfbe94] mt-1">{player.createdLevels}</span>
                   </div>
                 )}
            </div>
        </div>
      ...
      <div className="flex space-x-2 border-b border-white/10 pb-4 overflow-x-auto hide-scrollbar">
        <button 
          ...
          className={`px-6 py-3 rounded-lg font-bold text-sm whitespace-nowrap transition-all flex items-center gap-2 ${
            activeTab === "slayer" 
              ? "bg-[#cfbe94]/20 text-[#cfbe94] border border-amber-300/30" 
                 ...
                 {activeTab === "slayer" && player.completedLevelsList?.length > 0 && player.completedLevelsList.map((level, i) => (
                    <div key={i} className="flex items-center justify-between p-4 bg-white/5 border border-white/5 rounded-xl hover:bg-white/10 transition-colors">
                        <div className="flex items-center gap-4">
                            <div className="w-8 h-8 rounded-full bg-[#cfbe94]/20 text-[#cfbe94] flex items-center justify-center font-bold text-xs border border-amber-300/30">
                              {i+1}
                            </div>
                            <div>
                                <Link to={`/level/${level.id}`} className="font-bold text-white/90 hover:text-[#cfbe94] transition-colors text-sm md:text-base">{level.name}</Link>
                            </div>
                        </div>
                        <div className="text-right">
                             <div className="text-sm font-bold text-[#cfbe94]">{level.progress}%</div>
                             {level.url && (
                               <a href={level.url} target="_blank" rel="noopener noreferrer" className="text-[10px] text-white/40 hover:text-[#cfbe94] uppercase font-bold tracking-wider mt-1 block">Proof</a>
                             )}
                        </div>
                    </div>
                 ))}
                 ...
                 {activeTab === "creator" && player.createdLevelsList?.length > 0 && player.createdLevelsList.map((level, i) => (
                    <div key={i} className="flex items-center gap-4 p-4 bg-white/5 border border-white/5 rounded-xl hover:bg-white/10 transition-colors">
                        ...
                        <div className="font-bold text-white/90 text-sm md:text-base">
                            <Link to={`/level/${level.id}`} className="hover:text-[#cfbe94] transition-colors">{level.name}</Link>
                        </div>
                    </div>
                 ))}
              ...
              {activeTab === "slayer" && (
                <>
                  <div className="p-4 bg-white/5 rounded-xl border border-white/5 flex items-center gap-4">
                       <div className="p-3 bg-[#cfbe94]/10 rounded-lg text-[#cfbe94]">
                  ...
                  <div className="p-4 bg-white/5 rounded-xl border border-white/5 flex items-center gap-4">
                       <div className="p-3 bg-[#d8d0b6]/10 rounded-lg text-[#d8d0b6]">
                       ...
                       <div>
                           ...
                           <div className="text-lg font-bold mt-0.5 leading-tight">
                               {player.hardestDemonId ? (
                                  <Link to={`/level/${player.hardestDemonId}`} className="hover:text-[#cfbe94] transition-colors">{player.hardestDemon}</Link>
                               ) : player.hardestDemon}
                           </div>
                       </div>
                  </div>
                  ...
                  {player.hardestProgressPercent && player.hardestProgressPercent > 0 ? (
                    <div className="p-4 bg-white/5 rounded-xl border border-white/5 flex items-center gap-4">
                         <div className="p-3 bg-[#d8d0b6]/10 rounded-lg text-[#d2c89e]">
                         ...
                         <div>
                             ...
                             <div className="text-lg font-bold mt-0.5 leading-tight">
                                 {player.hardestProgressId ? (
                                    <Link to={`/level/${player.hardestProgressId}`} className="hover:text-[#cfbe94] transition-colors">{player.hardestProgressStr}</Link>
