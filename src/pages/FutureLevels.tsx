...
export default function FutureLevels() {
  ...
  const SortIcon = ({ columnKey }: { columnKey: keyof FutureLevel }) => {
    ...
    return <span className="ml-1 text-amber-400">{sortConfig.direction === 'asc' ? '↑' : '↓'}</span>;
  };
        ...
        <div className="flex w-full md:w-auto gap-3">
          <div className="relative flex-1 md:w-64 group">
            ...
            <input 
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl pl-10 pr-4 py-3 text-sm focus:outline-none focus:border-amber-500/50 transition-all backdrop-blur-sm text-slate-100 placeholder-white/30"
            />
          </div>
        </div>
      </div>
                      ...
                      <div className="grid grid-cols-[1fr_2fr_1fr_60px] gap-4 items-center py-4 px-6 rounded-xl hover:bg-white/5 transition-colors cursor-pointer border border-transparent backdrop-blur-sm group-hover:border-white/5">
                        ...
                        <div className="flex items-center gap-3">
                          {level.thumbnail ? (
                            ...
                            <div className="w-16 h-10 rounded-md bg-amber-500/10 border border-amber-500/20 shrink-0 flex items-center justify-center text-xs text-amber-400 font-mono">
                              FTR
                            </div>
                          )}
                          <div>
                        ...
                        {/* Crew */}
                        <div 
                          className="text-center font-semibold text-white/60 text-sm hover:text-orange-300 transition-colors"
                          onClick={(e) => {
                            e.stopPropagation();
                        ...
                        {/* Status */}
                        <div className="flex justify-center text-sm text-orange-300 font-medium">
                          {level.status}
                        </div>

                        {/* Action */}
                        <div className="flex justify-end text-white/20 group-hover:text-amber-400 transition-colors">
