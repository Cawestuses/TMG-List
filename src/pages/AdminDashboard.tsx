    ...
    <div className="min-h-screen pt-32 text-white px-4 md:px-8 max-w-7xl mx-auto">
      ...
      <div className="flex gap-4 mb-8 border-b border-white/10 pb-4">
        <button 
          ...
          className={`px-4 py-2 font-bold transition-colors ${activeTab === "levels" ? "text-amber-400 border-b-2 border-amber-400" : "text-white/60 hover:text-white"}`}
        >
          {t("admin.tabs.levels")}
        </button>
        <button 
          onClick={() => setActiveTab("future")}
          className={`px-4 py-2 font-bold transition-colors ${activeTab === "future" ? "text-amber-400 border-b-2 border-amber-400" : "text-white/60 hover:text-white"}`}
        >
          {t("admin.tabs.future")}
        </button>
        <button 
          onClick={() => setActiveTab("verifiers")}
          className={`px-4 py-2 font-bold transition-colors ${activeTab === "verifiers" ? "text-amber-400 border-b-2 border-amber-400" : "text-white/60 hover:text-white"}`}
        >
          {t("admin.tabs.verifiers")}
        </button>
        <button 
          onClick={() => setActiveTab("submissions")}
          className={`px-4 py-2 font-bold transition-colors ${activeTab === "submissions" ? "text-amber-400 border-b-2 border-amber-400" : "text-white/60 hover:text-white"}`}
        >
        ...
        {(isElderModer || isAdmin) && (
          <>
            <button 
              ...
              className={`px-4 py-2 font-bold transition-colors ${activeTab === "users" ? "text-amber-400 border-b-2 border-amber-400" : "text-white/60 hover:text-white"}`}
            >
              {t("admin.tabs.users")}
            </button>
            <button 
              onClick={() => setActiveTab("logs")}
              className={`px-4 py-2 font-bold transition-colors ${activeTab === "logs" ? "text-amber-400 border-b-2 border-amber-400" : "text-white/60 hover:text-white"}`}
            >
              {t("admin.tabs.logs")}
            </button>
          </>
        ...
        <button 
          ...
          className={`px-4 py-2 font-bold transition-colors ${activeTab === "changelog" ? "text-amber-400 border-b-2 border-amber-400" : "text-white/60 hover:text-white"}`}
        >
        ...
        <>
          <div className="flex flex-wrap gap-4 mb-6">
            ...
            <button 
              ...
              className="px-4 py-2 bg-amber-500/20 text-amber-400 border border-amber-500/30 rounded-lg text-sm font-bold uppercase"
            >
              {t("admin.levels.importJSON")}
            </button>
            <button 
              onClick={() => setIsSheetsModalOpen(true)}
              className="px-4 py-2 bg-orange-300/20 text-orange-300 border border-amber-300/30 rounded-lg text-sm font-bold uppercase"
            >
                ...
                {levels.map(level => (
                  <tr key={level.id} className="border-b border-white/5 hover:bg-white/5">
                    ...
                    <td className="p-4 text-right">
                      <button onClick={() => setIsEditingLevel({...level})} className="text-orange-300 hover:underline mr-4">{t("admin.levels.edit")}</button>
              ...
              <tbody>
                {futureLevels.map(future => (
                  <tr key={future.id} className="border-b border-white/5 hover:bg-white/5">
                    ...
                    <td className="p-4 text-orange-300 font-medium">{future.status}</td>
                    <td className="p-4 text-right">
                      <button onClick={() => setIsEditingFuture({...future})} className="text-orange-300 hover:underline mr-4">Edit</button>
              ...
              <tbody>
                {verifiers.map(verifier => (
                  <tr key={verifier.id} className="border-b border-white/5 hover:bg-white/5">
                    ...
                    <td className="p-4 text-right">
                      <button onClick={() => setIsEditingVerifier({...verifier})} className="text-orange-300 hover:underline mr-4">Edit</button>
              ...
              <tbody>
                {submissions.map(submission => (
                  <tr key={submission.id} className="border-b border-white/5 hover:bg-white/5">
                    ...
                    <td className="p-4 font-bold hover:text-orange-300">
                      <Link to={`/player/${encodeURIComponent(submission.username)}`} target="_blank">
                        {submission.username}
                      </Link>
                    </td>
                    <td className="p-4 hover:text-orange-300 font-medium">
                      {(() => {
                    ...
                    <td className="p-4 text-orange-300 font-medium">{submission.progress}%</td>
                    <td className="p-4 text-white/60 capitalize">{submission.status}</td>
                    <td className="p-4"><a href={submission.videoProof} target="_blank" rel="noopener noreferrer" className="text-orange-300 hover:underline">Link</a></td>
                    <td className="p-4 text-right">
                      {submission.status === "pending" && (
                        <>
                          <button onClick={() => handleAcceptSubmission(submission.id)} className="text-emerald-400 hover:underline mr-4">Accept</button>
                          <button onClick={() => handleRejectSubmission(submission.id)} className="text-[#f59e0b] hover:underline mr-4">Reject</button>
                        </>
                      )}
        ...
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-2">
            ...
            <input 
              ...
              className="bg-black border border-white/10 rounded-lg px-4 py-2 text-sm text-white w-full sm:w-64 focus:outline-none focus:border-amber-500/50"
            />
          </div>
          {loadingProfiles ? (
                  ...
                  {profiles
                    ...
                    .map(profile => (
                      <tr key={profile.id} className="hover:bg-white/5 transition-colors">
                        <td className="p-4 font-bold font-mono text-amber-400">{profile.id}</td>
                        <td className="p-4">
                        ...
                        <td className="p-4 flex items-center gap-2">
                          <select
                            ...
                            className="bg-black border border-white/10 rounded-lg p-2 text-xs text-white cursor-pointer focus:border-amber-500 disabled:opacity-50 disabled:cursor-not-allowed"
                          >
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-center mb-2">
            ...
            <div className="flex gap-2">
              <button 
                onClick={loadLogs} 
                className="px-4 py-2 bg-amber-500/20 text-amber-400 hover:bg-amber-500/30 border border-amber-500/35 text-xs font-bold rounded-lg uppercase tracking-wide transition-colors"
              >
                Refresh Logs
              </button>
            </div>
                ...
                {logsList.map(log => (
                  <tr key={log.id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="p-4 text-xs font-semibold">
                      <div className="text-amber-400 font-mono">@{log.moderatorUsername}</div>
              ...
              <tbody>
                {changelogs.map(log => (
                  <tr key={log.id} className="border-b border-white/5 hover:bg-white/5">
                    ...
                    <td className="p-4 text-right">
                      <button onClick={() => setIsEditingChangelog({...log})} className="text-orange-300 hover:underline mr-4">Edit</button>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 overflow-y-auto">
          <form onSubmit={saveLevel} className="bg-zinc-900 border border-white/10 p-6 rounded-2xl w-full max-w-xl">
                 ...
                 >
                   ...
                   {isProcessingImage ? (
                     <div className="flex flex-col items-center gap-2">
                       ...
                       <span className="text-xs text-amber-400 font-semibold uppercase tracking-wide">Compressing...</span>
                     </div>
                     ...
                     <div className="flex flex-col items-center gap-2">
                       <div className="p-3 bg-amber-500/10 text-amber-400 rounded-full border border-amber-500/20 transition-transform">
            ...
            <div className="flex justify-end gap-2">
              ...
              <button type="submit" className="px-4 py-2 bg-amber-600 hover:bg-amber-500 rounded font-bold text-white">Save</button>
            </div>
          </form>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-zinc-900 border border-white/10 p-6 rounded-2xl w-full max-w-2xl">
            ...
            <div className="flex justify-end gap-2">
              ...
              <button onClick={handleImportText} className="px-4 py-2 bg-amber-600 hover:bg-amber-500 rounded font-bold text-white">Import</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-zinc-900 border border-white/10 p-6 rounded-2xl w-full max-w-2xl">
            ...
            <div className="flex justify-end gap-2">
              ...
              <button onClick={handleImportFutureText} className="px-4 py-2 bg-amber-600 hover:bg-amber-500 rounded font-bold text-white">Import</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isSheetsModalOpen && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-zinc-900 border border-white/10 p-6 rounded-2xl w-full max-w-2xl">
            <h2 className="text-xl font-bold mb-2 font-heading text-orange-300">Sync Levels from Google Sheets (CSV)</h2>
            ...
            <div className="flex justify-end gap-2">
              ...
              <button 
                ...
                className="px-4 py-2 bg-gradient-to-r from-orange-400 to-orange-500 hover:-translate-y-0.5 rounded-xl font-bold text-white text-sm shadow-lg shadow-orange-400/15 transition-all disabled:opacity-50"
                disabled={isSyncingSheets}
              >
      ...
      {submissionWithComment && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 overflow-y-auto w-full h-full">
          <div className="bg-zinc-900 border border-white/10 p-6 rounded-2xl w-full max-w-md">
            <h2 className="text-xl font-bold mb-2 font-heading text-amber-400">
            ...
            <div className="space-y-2 mb-4 bg-black/40 p-3 rounded-lg border border-white/5 text-xs text-white/70">
              ...
              <p><strong>Video Proof:</strong> <a href={submissionWithComment.submission.videoProof} target="_blank" rel="noopener noreferrer" className="text-orange-300 underline hover:text-orange-200">Open Link</a></p>
            </div>
            ...
            <div className="mb-4">
              ...
              <textarea
                className="w-full bg-black border border-white/10 rounded-xl p-3 text-sm text-white focus:outline-none focus:border-amber-500 h-24"
